...
# Config Reponse File
sed -i -e "s|###GDBNAME###|$GDBNAME|g" $SCRIPTS_DIR/$CONFIG_RSP
sed -i -e "s|###ORACLE_SID###|$ORACLE_SID|g" $SCRIPTS_DIR/$CONFIG_RSP
sed -i -e "s|###ORACLE_PWD###|$ORACLE_PWD|g" $SCRIPTS_DIR/$CONFIG_RSP
...
# Create the Database
gosu oracle /bin/bash -c "dbca -silent -responseFile $SCRIPTS_DIR/$CONFIG_RSP"
